    ...
    for epoch in range(first_epoch, config.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                batch["pixel_values"] =batch["pixel_values"].to(target_device).to(weight_dtype)
                batch["input_ids"] =batch["input_ids"].to(target_device).to(weight_dtype).long()
                batch["attention_mask"] =batch["attention_mask"].to(target_device).to(weight_dtype).long()
